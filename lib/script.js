...
if (!window.localStorage.items) {
  ...
  balance.innerHTML = "".concat(window.localStorage.items, "$");
}

if (!window.localStorage.expense) {
  expense.innerHTML = "0$";
} else {
  expense.innerHTML = "".concat(window.localStorage.expense, "$");
}

if (!window.localStorage.balance) {
  remain.innerHTML = "0$";
} else {
  remain.innerHTML = "".concat(window.localStorage.balance);
}
...
if (!window.localStorage.list) {
  ...
  list.innerHTML += "".concat(window.localStorage.list);
}

var setBalance = function setBalance() {
  var reg = /[0-9.]/g;
  var l = amount.value.match(reg);
...
var expenseAdd = function expenseAdd() {
  var reg = /[0-9.]/g;
  var l = cost.value.match(reg);
  ...
  if (balance.innerHTML === '0$') {
    ...
    if (!l || l.length < cost.value.length) {
      ...
      if (remain.innerHTML.includes("-")) {
        window.localStorage.balance = +"-".concat(window.localStorage.balance.match(reg).join("")) - +cost.value;
        remain.innerHTML = "".concat(window.localStorage.balance, "$");
      } else {
      ...
      window.localStorage.expense = -+expense.innerHTML.match(reg).join("") + -+cost.value;
      expense.innerHTML = "".concat(window.localStorage.expense, "$");
      window.localStorage.list += "<li>".concat(name.value, " - ").concat(cost.value, "$</li>");
      list.innerHTML = window.localStorage.list;
      box.classList.add("active");
      box.innerHTML = "-".concat(cost.value, "$");
      setTimeout(function () {
